...
public class SmbUserMgr {
    ...
    public void saveUserInfo(String server, String username, String password){
        Log.i(TAG, "saveUserInfo server="+server+", username="+username);
    ...
    public String getUserInfo(String server){
        ...
        String s = getShareFromSmbPath(server);
        String v = mCache.get(s);
        if(v!=null) {
//            log("getUserInfo "+v);
            return v;
        }
    ...
    private String getShareFromSmbPath(String server) {
        //smb://172.21.174.215/smb/video/
        //count backslash
        int count = server.length() - server.replace("/", "").length();
        if(count>3){
