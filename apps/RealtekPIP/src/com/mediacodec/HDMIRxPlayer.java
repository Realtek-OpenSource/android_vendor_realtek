...
public class HDMIRxPlayer implements HDMIRxListener
{
    ...
    private final Runnable mPlayAfterStop = new Runnable() {
        @Override
        public void run() {
            if(mHDMIRX.isHDMIRxPlugged()) {
                mPlayCount=0;
                play();
            }
        }
    ...
    private BroadcastReceiver hdmiRxHotPlugReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
                ...
                Log.d(TAG, "onReceive: "+hdmiRxPlugged);
                if(hdmiRxPlugged) {
                    mPlayCount=0;
                    play();
    ...
    */
    ...
    public void onEvent(int eventId, int value) {
        if(eventId == HDMIRxListener.EVENT_HDMIRX_VIDEO_HOTPLUG) {
            Log.d(TAG, "[HDMIRx] onEvent: "+eventId+" value:"+value);
            if(value > 0) {
                mPlayCount = 0;
                play();
            }else{
    ...
    public HDMIRxPlayer(Context context, SurfaceView view, SurfaceHolder holder, int width, int height)
    {
        mContext = context;
        mSurfaceView = view;
    ...
    public boolean play()
    {
        ...
        if (mHDMIRX != null && mIsPlaying == false /**&& HDMIRXCallback.SurfaceIsReady==true**/)
        {
    ...
    public boolean stop()
    {
        ...
        if (mIsPlaying==true)
        {
            ...
            mHDMIRX.stop();
            mHDMIRX.release();
            // prepare again for getting hotplug event
            mHDMIRX.prepare();
            mHDMIRX.setListener(this);
            showStatusToast(true);
            mHandler.postDelayed(mPlayAfterStop,500);
        }
        else
        {
            rlt =false;
    ...
    public void release()
    {
        ...
        mHandler.removeCallbacks(mPlayAfterStop);
        if(mToast != null)
            mToast.cancel();
        if (mHDMIRX!= null)
        {
            mHDMIRX.setListener(null);
            mHDMIRX.release();
            mHDMIRX = null;
        }
    } 
