...
public class SMBFileDataProvider2 extends AbstractDataProvider
{
	...
	//http://127.0.0.1:8088/video/120311 Nine Muses(나인뮤지스) - TICKET(티켓).mp4
	private String encodeAll(String str){
	    if(str.startsWith("http")) {
            int idx = SmbUtils.nthOccurrence(str, '/', 2);
            if(idx<0) return Uri.encode(str);
            String[] splited = str.substring(idx).split("/");
            StringBuffer sb = new StringBuffer(str.substring(0, idx));
//            log("splited.length=" + splited.length);
            for (int i = 0; i < splited.length; i++) {
//                log("splited s=" + splited[i]);
                sb.append(Uri.encode(splited[i]));
...
//            return null;
        return encodeAll(mMntPath+SmbUtils.getPathFromSmbPath(smbPath));
    }
                ...
                List<FileIdBothDirectoryInformation> list = share.list(queryPath);
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        if ((EnumWithValue.EnumUtils.isSet(fi.getFileAttributes(), FILE_ATTRIBUTE_DIRECTORY)) &&
                            ...
                            mPlaylist_Photo.add(STREAM_PATH+encodeAll(filename));
                            ...
                            fileInfo.setSize(fileLen);
                            mPlaylist_Video.add(STREAM_PATH+encodeAll(filename));
                            ...
                            fileInfo.setSize(fileLen);
                            mPlaylist_Audio.add(STREAM_PATH+encodeAll(filename));
