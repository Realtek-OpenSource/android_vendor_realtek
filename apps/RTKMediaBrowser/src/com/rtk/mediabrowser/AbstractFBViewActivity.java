...
public abstract class AbstractFBViewActivity extends Activity implements UiListener
{
    ...
    private static final String PLAYLIST_FILE = "RTMediaBrowserPlaylist.txt";
    private static final String PLAYLIST_FILE_TITLE = "RTMediaBrowserPlaylist_title.txt";
    ...
    private AlertDialog mWarnImageworkerBusyDialog = null;
    NetworkUpdateCallback mNetworkUpdateCallback;
...
//                        Toast.makeText(AbstractFBViewActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    private class NetworkUpdateCallback extends ConnectivityManager.NetworkCallback {
        @Override
...
//        mIntentFilter.addAction(EthernetManager.ETHERNET_STATE_CHANGED_ACTION);
    ...
    public void onPause() {
        ...
        if(mWarnImageworkerBusyDialog != null) {
            mWarnImageworkerBusyDialog.dismiss();
            mWarnImageworkerBusyDialog = null;
        }
        mState = S_PAUSE;
    }
    @Override
    ...
    public boolean dispatchKeyEvent(KeyEvent event)
    {
        ...
        if(mGenericContentViewFragment.mIgnoreKeyEvent) {
            if(action == KeyEvent.ACTION_DOWN) {
                if (mWarnImageworkerBusyDialog == null) {
                    mWarnImageworkerBusyDialog = new AlertDialog.Builder(this)
                            .setTitle("Warning")
                            .setMessage("Imageworker is busy on video frame decode.")
                            ...
                            .create();
                    mWarnImageworkerBusyDialog.setOnCancelListener(new OnCancelListener() {
                        @Override
                        public void onCancel(DialogInterface dialog) {
                            mWarnImageworkerBusyDialog = null;
                        }
                    });
                    mWarnImageworkerBusyDialog.show();
                }
            }
    ...
    private void getDataProvider(String path, int SortMode)
    {
        ...
        {   
			...
			else if(mBrowserType == MbwType.TYPE_DEVICE_SMB)
			{
                ...
                String password="";
                String[] infos = SmbUtils.tryGetSmbLoginInfo(this, path);
                if(infos!=null) {
                    if (infos.length >= 1)
                        user = infos[0];
                    if (infos.length >= 2)
...
//Don't ask stop query in some Browser type, because we need query work complete done for creating playlist(ex:USB, DB video....), should ask don't update forground list and UI instead. But
    ...
    private void grantReadPermission(Context ctx, Intent intent, String[] urls) {
        ...
        if(intent.getComponent()==null) {
            List<ResolveInfo> resolvedIntentActivities = ctx.getPackageManager()
    ...
    private void onItemClickedPostAction(DataProviderStruct stru){
        ...
        switch (stru.GetFileTypeAt)
        {
            ...
            {
                Intent intent = new Intent();
                intent.setComponent(new ComponentName("com.rtk.mediabrowser.installer", "com.rtk.mediabrowser.installer.Installer"));
                String filePath = stru.GetDataAt;
                if (!filePath.startsWith("http://"))
                    filePath = "file://" + filePath;
                Uri uri = Util.getUri(this, filePath);
                intent.putExtra("EXTRA_FILE_PATH", stru.GetDataAt);
            ...
            case FileFilterType.DEVICE_FILE_PHOTO:
            {
                ...
                if (size >100)
                {
                    String playtlistFilePath = PlaylistToFile(PLAYLIST_FILE, playlist);
                    intent.putExtra("PlayListAddress", Util.getUri(this, playtlistFilePath).toString());
                    String playtlistTitlePath = PlaylistToFile(PLAYLIST_FILE_TITLE, playlist_title);
                    intent.putExtra("PlayListTitleAddress", Util.getUri(this, playtlistTitlePath).toString());
                }
                else
                {
            ...
            case FileFilterType.DEVICE_FILE_VIDEO:
            {
                ...
                if (mBrowserType == MbwType.TYPE_DEVICE_DMS||mBrowserType == MbwType.TYPE_DEVICE_SMB)
                    ...
                    intent.putExtra("SourceFrom","Local");
                if(mBrowserType==MbwType.TYPE_DEVICE_DMS){
                    intent.putExtra(EXTRA_DVDPLAYER_EXTENSION, "forceSeek");
                }
                intent.setAction(Intent.ACTION_VIEW);
                intent.setData(uri);
...
//                log("playlist="+Arrays.toString(playlist.toArray()));
            ...
            case FileFilterType.DEVICE_FILE_AUDIO:
            {
                if(MediaBrowser.sNoMusicApp) {
                    Toast.makeText(this, "no music app", Toast.LENGTH_SHORT).show();
                    break;
                }
                ...
                Intent intent = Util.makeMusicPlayerIntent();
                ...
                if (!SINGLE_MUSIC_PLAYBACK)
                {
                    ...
                    if (size >100)
                    {
                        ...
                        intent.putExtra("PlayListAddress", "file://"+playtlistFilePath);
                        String playtlistTitlePath = PlaylistToFile(PLAYLIST_FILE_TITLE, playlist_title);
                        intent.putExtra("PlayListTitleAddress", "file://"+playtlistTitlePath);
                    }
                    else
                    {
                        String[] playlistArray = new String[playlist.size()];
    ...
    private String PlaylistToFile(String FileName, ArrayList<String> playlist)
    {
        try {
            //Delete old file and create new now
            File outputDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS);
            File file = new File(outputDir, FileName.substring(FileName.lastIndexOf(File.separator)+1));
            Log.e(TAG, "PlaylistToFile "+ file.toString());
            if (file.exists())
                file.delete();
            file.createNewFile();
