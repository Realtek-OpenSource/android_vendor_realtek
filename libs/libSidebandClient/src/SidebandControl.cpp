...
SidebandControl::SidebandControl(
    ...
    : mClient(client), mHandle(handle), mGraphicBufferProducer(gbp), mSidebandStream(NULL)
{
}
...
status_t SidebandControl::requestHwSyncByFd(int32_t* resource, int audioHwSync) {
    status_t err = validate();
    if (err < 0) return err;
...
status_t SidebandControl::setHackSideband(bool use) {
    status_t err = validate();
    if (err < 0) return err;
    return mClient->setHackSideband(mHandle, use);
}
...
status_t SidebandControl::createSidebandStream(native_handle_t ** handle) {
    status_t err = validate();
    if (err < 0) return err;
    return mClient->createSidebandStream(mHandle, handle);
}
...
status_t SidebandControl::getSidebandInfo(String8 &info) {
    status_t err = validate();
    if (err < 0) return err;
    return mClient->getSidebandInfo(mHandle, info);
}
