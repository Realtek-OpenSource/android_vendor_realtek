...
public class MediaBrowser extends Activity 
{
	...
	private boolean  mDisableDLNA = true;
	...
	public static boolean sNoMusicApp;
    ...
    public void onCreate(Bundle savedInstanceState) 
	{
        ...
        if(mVoiceBtn!=null){
			...
			List<ResolveInfo> infos = Util.queryIntentActivities(this, voiceSearchIntent);
			if (infos.size() > 0) {
		...
		//Check is music player app exists
		List<ResolveInfo> infos = Util.queryIntentActivities(this, Util.makeMusicPlayerIntent());
...
//		filter.addAction(EthernetManager.ETHERNET_STATE_CHANGED_ACTION);
		...
		if(intent!=null){
			...
			if(Intent.ACTION_GET_CONTENT.equals(action)){
				...
				if(intentUri.getScheme().equals("rtkrecommend")){
					//just open MediaBrowser
				} else {
	...
	private void tryInitDvdPlayer() {
		if(UseRtMediaPlayer.getInstance(this).getUseRtPlayer()) {
			log("tryInitDvdPlayer");
