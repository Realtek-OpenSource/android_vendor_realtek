...
public class VideoDecodeThread extends Thread {
            ...
            if (mime.startsWith("video/")) {
                ...
                try {
                      if(audioSessionID != -1)
                      {
                         ...
                         if(audioSessionID == 0)
                             format.setInteger("tunnel-no-audio", 1);
                      }

                      if((format.getInteger(MediaFormat.KEY_WIDTH)*format.getInteger(MediaFormat.KEY_HEIGHT)) > (1920*1080) && scale == 1)
                      {
                          format.setInteger("eWid", 1920);
                          format.setInteger("eHei", 1080);
                      }
    ...
    private void setupDecoderCallback(Handler handle)
    {
        ...
        {
            if(mStartRealease == true || mIsSeeking == true)
                return;
            if (!isEOS) { 
        ...
        public void onError(MediaCodec codec, MediaCodec.CodecException e)
        {
            ...
            if(isCodecErr)
            {
                ...
                //ErrPTS = true; //Workaround for demo
            }
        }
        }, handle);
    }
    ...
    public synchronized long seekTo(long position) {
        ...
        Log.d(TAG, "[expect]video seekTo : " + position);
        extractor.seekTo(position, MediaExtractor.SEEK_TO_CLOSEST_SYNC);
        mSeekToPTS = extractor.getSampleTime();
        Log.d(TAG, "[actual]video seekTo : " + mSeekToPTS);

        startMs = startMs + (curTimeStamp - mSeekToPTS) / 1000 + 100;
