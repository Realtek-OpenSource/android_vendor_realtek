...
{	
    ...
    private ArrayList<FileInfo> mBDFileList_bg =  new ArrayList<FileInfo>();
...
//            log("mFileList_bg:" + Arrays.toString(mFileList_bg.toArray()));
    ...
    private void CreateBDFileListArray() {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if ((files[i].isDirectory()) &&
                ...
                if(entry!=null){
                    ...
                    fileInfo.setSize(files[i].length());
                    synchronized (mBDFileList_bg) {
                        mBDFileList_bg.add(fileInfo);
                    }
        ...
        Collections.sort(mBDFileList_bg, new SortByType());
        for(FileInfo fi : mBDFileList_bg){
            mPlaylist_BD.add(fi.path);
            mNamelist_BD.add(fi.fileName);
        }
        mBDQueryDone = true;
    }
