...
public class BluetoothDevicePairer {
     ...
     */
    public static final int DELAY_AUTO_PAIRING = 5 * 1000;
    /**
    ...
    private final OpenConnectionCallback mOpenConnectionCallback = new OpenConnectionCallback() {
        public void succeeded() {
            Log.d(TAG,"mOpenConnectionCallback succeeded" );
            setStatus(STATUS_NONE);
        }
        public void failed() {
            Log.d(TAG,"mOpenConnectionCallback failed" );
    ...
    public BluetoothDevicePairer(Context context, EventListener listener) {
        ...
        mHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                Log.d(TAG,"BluetoothDevicePairer msg.what="+msg.what);
                switch (msg.what) {
                    case MSG_PAIR:
                        startBonding();
                        break;
    ...
    private void addBluetoothDeviceCriteria() {
        Log.d(TAG,"addBluetoothDeviceCriteria");
    ...
    public void clearDeviceList() {
        if (DEBUG) Log.d(TAG,"clearDeviceList");
        doCancel();
    ...
    public void cancelPairing() {
        if (DEBUG) Log.d(TAG,"cancelPairing");
        mAutoMode = false;
        doCancel();
    }
    ...
    public List<BluetoothDevice> getAvailableDevices() {
        if (DEBUG) Log.d(TAG,"getAvailableDevices");
    ...
    private void startPairing(BluetoothDevice device, boolean isManual) {
        Log.d(TAG,"startPairing isManual="+isManual);
        ...
        if (mAutoMode) {
            mHandler.sendEmptyMessageDelayed(MSG_PAIR, DELAY_AUTO_PAIRING);
        } else {
    ...
    private void updateListener() {
        if (DEBUG) Log.d(TAG,"updateListener");
        if (mListener != null) {
    ...
    private void updatePairingState() {
        Log.d(TAG,"updatePairingState");
        if (mAutoMode) {
            Log.d(TAG,"updatePairingState AUTO");
    ...
    private BluetoothDevice getAutoPairDevice() {
        if (DEBUG) Log.d(TAG,"getAutoPairDevice");
        ...
        if (inputDevices.size() == 1) {
            Log.d(TAG,"getAutoPairDevice device="+inputDevices.get(0).getName());
    ...
    private void registerLinkStatusReceiver() {
        if (DEBUG) Log.d(TAG,"registerLinkStatusReceiver");
    ...
    private void unregisterLinkStatusReceiver() {
        if (DEBUG) Log.d(TAG,"unregisterLinkStatusReceiver");
