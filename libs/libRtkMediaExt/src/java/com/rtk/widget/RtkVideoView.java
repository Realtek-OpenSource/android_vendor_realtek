...
public class RtkVideoView extends SurfaceView
    ...
    private String TAG = "RtkVideoView_Ext";
    ...
    private RtkMediaPlayerWrap mMediaPlayer = null;
    ...
    private int mForceUseRTMediaPlayer = RtkMediaPlayer.FORCE_NONE;
...
//============================================
    ...
    private void openVideo() {
        ...
        release(false);
        try {
            mMediaPlayer = new RtkMediaPlayerWrap(mForceUseRTMediaPlayer);
            ...
            mMediaPlayer.setParameter(RtkMediaPlayer.KEY_PARAMETER_VIDEO_BUFFER_SIZE, value);
            //if (mUri.toString().startsWith("http") == false)
                //mMediaPlayer.setAutoScanExtSubtitle(mAutoScanExtSubtitle);

            if(isRTMediaPlayer() && mVideoScaledSize){
    ...
    private OnSeekResultListener mSeekResultListener =
        new OnSeekResultListener() {
    ...
    private OnSpeedChangedListener mSpeedChangedListener =
        new OnSpeedChangedListener() {
    ...
    */
    ...
    public RtkMediaPlayer.TrackInfo[] getRtkTrackInfo() throws IllegalStateException {
        String ret;
        RtkMediaPlayer.TrackInfo[] trackInfo = null;
        if (isInPlaybackState())
        {
            ret="getTrackInfo ok";
            trackInfo =  mMediaPlayer.getRtkTrackInfo();
        } else {
    ...
    */
    public void getAudioTrackInfo(boolean bForceUpdate)
    {
        if (mMediaPlayer!=null) {
            mMediaPlayer.getAudioTrackInfo(bForceUpdate);
        }
    }
    ...
    */
    ...
    public int getAudioSessionId() {
        if (mAudioSession == 0) {
            MediaPlayer foo = new RtkMediaPlayerWrap(mForceUseRTMediaPlayer);
    ...
    public boolean isRTMediaPlayer() {
        if(mForceUseRTMediaPlayer == RtkMediaPlayer.FORCE_RT_MEDIAPLAYER) {
            return true;
        }
        return false;
    }
