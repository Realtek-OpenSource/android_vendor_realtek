...
import com.realtek.DataProvider.SMBFileDataProvider2;
...
public class SmbBrowser {
    ...
    private SMBFileDataProvider2 mntDataProvider = null;
...
//                mDomain = segments[0];
                ...
                try {
                    if(Const.GUEST.equals(mUser) && "".equals(mPassword)) {
                        SmbUtils.testAnonymousLogin(path);
                    ...
                    if(e.getStatus()==NtStatus.STATUS_LOGON_FAILURE) {
                            Message msg = Message.obtain(mHandler, SmbBrowser.SmbAuthFailed, 0, 0, null);
...
//            }
        ...
        else if (queryType == SMB_QUERY_FOLDER)
        {
            ...
            if (segments.length == 2) // Move into the smb share folder
            {
                final int METHOD = 2;
                if(METHOD==2){
                    log("SMB_QUERY_FOLDER by smbj, start StreamServer");
                    ...
                    mMountPath = StreamServer.getHttpPrefix();
                    log("SMB_QUERY_FOLDER by smbj, "+mDomain+", "+mUser);
                    try {
                        StreamServer.getInstance(smbPath, mDomain, mUser, mPassword).start();
...
//                        mntDataProvider = new DeviceFileDataProvider(mountPath,
            ...
            {
                if(true){
                    log("SMB_QUERY_FOLDER query deep folder by jcifs");
                    //get smb share root
                    mMountPath = StreamServer.getHttpPrefix();
                    Log.d(TAG, "SMB_QUERY_FOLDER mSmbPath = " + mSmbPath+", mMountPath="+mMountPath);
                    if (!mPath.equals(path)) {
                        if (mntDataProvider != null) {
                            mntDataProvider.StopQuery();// Stop Current Query.
                        }
                        log("SMB_QUERY_FOLDER query deep folder mUser:" + mUser);
//                    mntDataProvider =  new DeviceFileDataProvider(mntPath,
                    ...
                    log("SMB_QUERY_FOLDER query deep folder:" + mMountPath);
