...
//        sInstance = SingletonContext.getInstance();
    ...
    public static SMBClient getClient() {
        ...
        sClient = new SMBClient(SmbConfig.createDefaultConfig());
        return sClient;
    }
...
//            log("getUserInfo userInfo="+userInfo);
    ...
    static void testAnonymousLogin(String smbPath) throws SMBApiException {
        String ip = smbPath.substring(smbPath.indexOf("//") + 2, nthLastIndexOf(smbPath, '/', 1));
        log("testAnonymousLogin ip=" + ip);
        SMBClient client = new SMBClient(SmbConfig.createDefaultConfig());
        try (Connection c = client.connect(ip);
             Session s = c.authenticate(AuthenticationContext.anonymous())) {
