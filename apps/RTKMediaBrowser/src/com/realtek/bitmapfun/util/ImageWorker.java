...
//        Picasso.with(mContext).load(path).into(imageView);
    ...
    protected abstract Bitmap processBitmap(int resId);
    protected abstract Bitmap processBitmap(Object data, int type);
    ...
    private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
                    ...
                    && !mExitTasksEarly)
            {
				...
				else if(type == FileFilterType.DEVICE_FILE_VIDEO)
				{
                    ...
                    if (bitmap == null && !isCancelled() && !mExitTasksEarly) {//For save system loading, add default icon to cache
					    ...
					    bitmap = processBitmap(R.drawable.icon_default_preview);
                    }
				}
				else	
				{
		...
		private Bitmap GetID3BitmapViaMediaMetadataRetriever(String FileName)
		{
			...
			try
			{
				...
				{
					bitmap = processBitmap(m_MMR_Picture, OBJECT_TO_BYTEARRAY);
				}
			} catch (Exception e) {
        ...
        private Bitmap GetVideoThumbnailViaDB(String FileName)
        {
            ...
            try {
                ...
                if (cursor.moveToNext())
                {
                   ...
                   Cover = processBitmap((Object)CoverPath);
                }
            } catch (Exception e) {
...
//              log("processBitmap begin");
                bitmap = processBitmap(framebitmap, OBJECT_TO_BITMAP);
                framebitmap.recycle();
	...
	*/
    ...
    public Bitmap loadRecommendationImage(int id, String title, Object data, Object coverPath) {
        ...
        if (bitmap == null) {
            ...
            if(FileName.length()>7 && !(FileName.startsWith("http://"))) {
                if (coverPath != null) {
                    bitmap = processBitmap((Object)coverPath);
                    if (bitmap == null) {
