...
public:
    ...
    virtual status_t requestHwSyncByFd(int32_t* resource, int audioHwSync) {
        ...
        remote()->transact(REQUEST_HWSYNC_BY_FD, data, &reply);
        *resource = static_cast<int32_t>(reply.readInt32());
        return reply.readInt32();
    }
    ...
    virtual status_t getSidebandInfo(const sp<IBinder>& handle, String8 &info) {
        ...
        data.writeInterfaceToken(ISidebandClient::getInterfaceDescriptor());
        data.writeStrongBinder(handle);
        remote()->transact(GET_SIDEBANDINFO, data, &reply);
        info = reply.readString8();
        return reply.readInt32();
    }
   ...
   virtual status_t setHackSideband(const sp<IBinder>& handle, bool use) {
        ...
        data.writeBool(use);
        data.writeStrongBinder(handle);
        remote()->transact(SET_HACKSIDEBAND, data, &reply);
        return reply.readInt32();
   }
   virtual status_t controlBW(void) {
       ALOGI("[%s:%d]", __FUNCTION__, __LINE__);
       Parcel data, reply;
       data.writeInterfaceToken(ISidebandClient::getInterfaceDescriptor());
       remote()->transact(CONTROL_BW, data, &reply);
       return reply.readInt32();
   }
};
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
     switch(code) {
        ...
        case REQUEST_HWSYNC_BY_FD: {
            CHECK_INTERFACE(ISidebandClient, data, reply);
            int32_t resource;
            int audioHwSync = dup(data.readFileDescriptor());
            status_t result = requestHwSyncByFd(&resource, audioHwSync);
            reply->writeInt32(static_cast<int32_t>(resource));
            reply->writeInt32(static_cast<int32_t>(result));
            return NO_ERROR;
        }
        ...
        case GET_SIDEBANDINFO: {
            CHECK_INTERFACE(ISidebandClient, data, reply);
            String8 info;
            status_t result = getSidebandInfo(data.readStrongBinder(), info);
            reply->writeString8(info);
            return NO_ERROR;
        }
        case SET_HACKSIDEBAND: {
            CHECK_INTERFACE(ISidebandClient, data, reply);
            bool use = data.readBool();
            reply->writeInt32( setHackSideband (data.readStrongBinder(), use));
            return NO_ERROR;
        }
        case CONTROL_BW: {
            CHECK_INTERFACE(ISidebandClient, data, reply);
            status_t result = controlBW();
            return NO_ERROR;
        }
        default:
