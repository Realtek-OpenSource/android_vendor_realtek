...
public class RealtekSoftwareUpdaterActivity extends Activity implements OnScrollListener {
    ...
    private static String mOTAXMLUrl = null;
    ...
    */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (null == mOTAXMLUrl || (mOTAXMLUrl.length() == 0)) {
            mOTAXMLUrl = SystemProperties.get("rtk.ota.xmlurl");
        }
    ...
    */
    ...
    private void doOTASearch() {
        ...
        if (mOTAXMLUrl != null && !"".equals(mOTAXMLUrl)) {
            Log.d(TAG, "OTAXML URL=" + mOTAXMLUrl);
    ...
    private void doUSBSearch() {
        ...
        for(File folder: sSearchPath) {
            ...
            for(File file: files) {
                //Log.v(TAG, "search file1: " + file.toString());
                //String patternZip = "(.*).zip";

                String patternZip = "(.*)ota(.*).zip";  // simple filename checking: we expect rtk_____.zip
                ...
                if(file.isFile() && file.toString().matches(patternZip)) {   // .zip
                    ...
                    Log.v(TAG, "target found: " + file.toString());
               }
            }
        }
            ...
            try {
                    ...
                    if(mSoftwareFilesInFullPath.get(position).getType() == RECOVERY_SOFTWARE_TYPE_USB) {
                        if(SystemProperties.getInt("rtk.vmx.boot",0)==1){
                            ...
                            dst = new File("/mnt/usb/" + mSoftwareFilesInFullPath.get(position).getShortFilename());
                        }
                    } else if(mSoftwareFilesInFullPath.get(position).getType() == RECOVERY_SOFTWARE_TYPE_OTA) {
                        dst = new File("/data/ota_package", mSoftwareFilesInFullPath.get(position).getShortFilename());
                ...
                if(mSoftwareFilesInFullPath.get(position).getType() == RECOVERY_SOFTWARE_TYPE_OTA) {  // start downloading file from server
                    new DownloadFileFromURL().execute(mSoftwareFilesInFullPath.get(position).getFilename(), dst.toString());
                }
    ...
    public class RetrieveFeedAsync extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                ...
                //Log.d(TAG, "xml content length=" + content.length());
                // parsing
        ...
        private ProgressDialog getProgressDialog() {
            final ProgressDialog progressDialog = new ProgressDialog(RealtekSoftwareUpdaterActivity.this);
            progressDialog.setIndeterminate(true);
            progressDialog.setCancelable(false);
            progressDialog.setTitle("OTA");
            progressDialog.setMessage("OTA XML parsing");
...
//        int displayLogControl = 5;
        ...
        protected String doInBackground(String... f_url) {
            int count;
            try {
                Log.d(TAG, "downloading filename: " + f_url[1]);
                ...
                Log.d(TAG, "downloading length of file: " + lenghtOfFile/(1024*1024) + "MB");

                dst = new File(f_url[1]);
                long freeSpace = dst.getParentFile().getUsableSpace();
                Log.d(TAG, "free space: " + freeSpace/(1024*1024) + "MB");
                if (lenghtOfFile > freeSpace) {
                    Log.d(TAG, "ERROR!!!!! disk space is not enough!");
